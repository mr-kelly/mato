...
fn test_daemon_terminal_persistence() {
    ...
    
    {
        ...
        match response {
            ServerMsg::Screen { content, .. } => {
                assert!(!content.lines.is_empty(), "Screen should still have content after reconnect");
                
                // Check if our text is still there
                let has_hello = content.lines.iter().any(|line| {
                    line.cells.iter().any(|cell| cell.ch == 'H' || cell.ch == 'e')
                });
                
                assert!(has_hello, "Content should persist across reconnections");
                println!("Second connection: Content persisted! Got {} lines", content.lines.len());
            }
            _ => panic!("Expected Screen response"),
        }
    }
...
fn test_daemon_multiple_tabs() {
    ...
    for tab_id in &[&tab1, &tab2] {
        ...
        match response {
            ServerMsg::Screen { content, .. } => {
                assert!(!content.lines.is_empty(), "Tab {} should have content", tab_id);
            }
            _ => panic!("Expected Screen response"),
        }
    }
    ...
    // Get screen content after resize - should still be there!
    {
        ...
        match response {
            ServerMsg::Screen { content, .. } => {
                ...
                println!("After resize: {} lines - Content preserved!", content.lines.len());
            }
            _ => panic!("Expected Screen response"),
        }
    }
