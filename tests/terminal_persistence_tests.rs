...
fn test_terminal_content_persistence() {
    ...
    assert!(
        !screen1.lines.is_empty(),
        "Screen should have content after writing"
    );
    ...
    let has_hello = screen1.lines.iter().any(|line| {
        line.cells
            .iter()
            .any(|cell| cell.ch == 'H' || cell.ch == 'e' || cell.ch == 'l')
    });
...
fn test_resize_preserves_content_when_size_unchanged() {
    ...
    assert_eq!(
        screen_before.lines.len(),
        screen_after.lines.len(),
...
fn test_multiple_writes_accumulate() {
    ...
    assert!(
        non_empty_lines >= 3,
        "Should have at least 3 lines of content, got {}",
        non_empty_lines
    );
}
...
fn test_pty_survives_multiple_get_screen_calls() {
    ...
    for i in 0..10 {
        ...
        assert!(
            ...
            "Screen should have content on call {}",
            i
        );
    }
}
